...
Puppet::Type.type(:directadmin_reseller_package).provide :directadmin, :parent => Puppet::Provider::Directadmin do  
  def convert_package
    ...
    resource[:userssh] == :on ? (package[:userssh] = "ON") : (package[:userssh] = "OFF")
  ...
  def create   
    connect
    convert_package
    set_package = self.class.post('CMD_API_MANAGE_RESELLER_PACKAGES', @package_details)    
  end

  def destroy
    connect
    self.class.post('CMD_API_MANAGE_RESELLER_PACKAGES',
      :delete     => "Delete",
  ...
  def exists?
    connect 
    packages = self.class.query('CMD_API_PACKAGES_RESELLER')
  ...
  def get_package_values
    connect
    package = self.class.query('CMD_API_PACKAGES_RESELLER',
  ...
  def oversell
    package = @package_values
    return package["oversell"].downcase
  end
  ...
  def serverip
    package = @package_values
    return package["serverip"].downcase
  end
  ...
  def userssh
    package = @package_values
    return package["userssh"].downcase
  end
